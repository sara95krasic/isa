...
public interface TheaterRepository extends Repository<Theater, Long> {
	...
	@Query("select distinct p.title as title, p.description as description, p.runtime as runtime, p.id as id, p.director as director, p.poster as poster, p.id as id " +
			"from ProjectionDate pd left outer join pd.projection p left outer join pd.hall h " +
